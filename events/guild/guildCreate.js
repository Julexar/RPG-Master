...
class Event {
    ...
    async run(guild) {
        ...
        client.database.Server.add(server).then(client.writeDevLog).catch(client.logDevError);

        client.database.Server.logs
            .add(server)
        ...
        client.database.Server.members
            .getAll(server)
            .then(async members => {
                    ...
                    if (dbServer.print_logs) {
                        if (dbServer.log_channelId) {
                            ...
                            throw new NotFoundError(
                                'No Logchannel found',
                                `This Server has no Logchannel defined, please set one with \`/server setchannel log\``
                            );
                    }
                } catch (err) {
                    client.logServerError(server, err);
        ...
        await guild.commands
            ...
            .then(commands => {
                client.writeServerLog(
                    server,
                    `Successfully registered ${commands.size} Commands in Server - Attempting to write Server Commands to Database...`
                );
                ...
                commands.forEach(async command => {
                    ...
                    try {
                        ...
                        client.writeServerLog(
                            server,
                            `Found ${restrictions.length} Restrictions for Command /${command.name} - Attempting to add them to the Command...`
                        );
