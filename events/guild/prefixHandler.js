        ...
        client.database.getPrefixes(message.guild)
          .then(prefixes => {
            if (prefixes) {
                  ...
                  if (cmd.args) {
                    if (!cmd.optional) {
                      if (args.length < cmd.usage.length) {
                        ...
                        client.database.writeLog(message.guild, `${prefix}${cmd.name} was triggered by ${message.author.username}\nArguments: ${args.join(", ")}`)
              ...
              client.database.writeLog(message.guild, "Error 404: No Prefixes found")
                .then(message.reply({
                  embeds: [
                    new EmbedBuilder()
                      .setColor("Red")
          ...
          .catch(err => {
            client.database.writeLog(message.guild, `${err}`)
              .then(message.reply({
                embeds: [
                  new EmbedBuilder()
                    .setColor("Red")
