...
class Command {
    ...
    async run(client, interaction) {
        ...
        switch (option.getSubcommand()) {
            case 'armor':
                row1.addComponents(new StringSelectMenuBuilder().setCustomId('armsel').setMaxValues(1).setPlaceholder('No Armor selected...'));
                rows.push(row1);
                client.database
                    .getArmor(server)
                    .then(async (armors) => {
                ...
                break;
            case 'class':
                row1.addComponents(new StringSelectMenuBuilder().setCustomId('csel').setMaxValues(1).setPlaceholder('No Class selected...'));
                rows.push(row1);
                client.database
                    .getClass(server)
                    .then(async (classes) => {
                        ...
                        collector.on('collect', async (i) => {
                            if (i.customId == 'csel') {
                                client.database
                                    .getClass(server, { id: Number(i.value[0]) })
                                    .then(async (clas) => {
                                        let traits = '';
                                        let prevlvl;
