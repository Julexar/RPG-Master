...
class Command {
    ...
    async noteRemover(client, interaction, type, member, noteId) {
        ...
        if (!noteId) {
            ...
            rows.push(row);
            if (type=="server") {
                client.database.getServerNote(interaction.guild, member.user)
                    .then(async (notes) => {
                        let count = 0;
                        let num = 0;
                        for (const note of notes) {
                        ...
                        collector.on("collect", async (i) => {
                            if (i.customId=="selnote") {
                                const mes = await i.deferReply();
                                client.database.getServerNote(interaction.guild, member.user, {id: Number(i.values[0])})
                                    .then(async (note) => {
                                        ...
                                        col.on("collect", async (j) => {
                                            ...
                                            if (j.customId=="confirm") {
                                                await client.database.remServerNote(interaction.guild, member.user, {id: note.id})
                                                    .then(async (m) => {
                                                        await client.database.writeLog(interaction.guild, m)
                                                    ...
                                                    .catch(async (err) => {
                                                        await client.database.writeLog(interaction.guild, `${err}`)
                                        ...
                                        col.on("end", async (collected) => {
                                            if (collected.size == 0) {
                                                ...
                                                client.database.writeLog(interaction.guild, `Collected ${collected.size} Interactions`)
                                                    .then(m => client.database.writeDevLog(m))
                                                    .catch(err => client.database.writeDevLog(`${err}`));
                                            }
                                            setTimeout(async () => {
                        ...
                        collector.on("end", async (collected) => {
                            if (collected.size == 0) {
                                ...
                                await client.database.writeLog(interaction.guild, `Collected ${collected.size} Interactions`)
                                    .then(m => client.database.writeDevLog(m))
                                    .catch(err => client.database.writeDevLog(`${err}`));
                            }
                            setTimeout(async () => {
                ...
                client.database.getGlobalNote(member.user)
                    .then(async (notes) => {
                        ...
                        collector.on("collect", async (i) => {
                            if (i.customId=="selnote") {
                                const mes = await i.deferReply();
                                client.database.getGlobalNote(member.user, {id: Number(i.values[0])})
                                    .then(async (note) => {
                                        const embed = new EmbedBuilder()
                                            .setColor("Yellow")
                                            .setDescription(note.content)
                                        ...
                                        col.on("collect", async (j) => {
                                            const messag = await j.deferReply();
                                            if (j.customId=="confirm") {
                                                await client.database.remGlobalNote(member.user, {id: note.id})
                                                    .then(async (m) => {
                                                        await client.database.writeLog(interaction.guild, m)
                                                            .then(message => client.database.writeDevLog(message))
                                                            .catch(err => client.database.writeDevLog(`${err}`));
            ...
            let note;
            if (type=="server") {
                note = await client.database.getServerNote(interaction.guild, member.user, {id: noteId})
                    .catch(async (err) => {
                ...
                if (note) {
                    ...
                    collector.on("collect", async (i) => {
                        ...
                        if (i.customId=="confirm") {
                            client.database.remServerNote(interaction.guild, member.user, note)
                                .then(async (m) => {
                ...
                note = await client.database.getGlobalNote(member.user, {id: noteId})
                    .catch(async (err) => {
                        client.database.writeDevLog(await client.database.writeLog(interaction.guild, `${err}`).catch(err1 => client.database.writeDevLog(`${err1}`)));
                        if (String(err).includes("Error 404")) {
                            await interaction.reply({
                ...
                if (note) {
                    ...
                    collector.on("collect", async (i) => {
                        const mes = await i.deferReply();
                        if (i.customId=="confirm") {
                            client.database.remGlobalNote(member.user, note)
                                .then(async (m) => {
                                    client.database.writeDevLog(await client.database.writeLog(interaction.guild, m).catch(err => client.database.writeDevLog(`${err}`)));
                                    await mes.edit({
                                        content: `${m}`,
    ...
    noteEditor(client, interaction, type, member, noteId) {
        ...
        if (type=="server") {
            client.database.getServerNote(interaction.guild, member.user, {id: noteId})
                .then(async (note) => {
                    if (note.title) {
                    ...
                    collector.on("collect", async (i) => {
                        ...
                        if (i.customId=="title") {
                            ...
                            const privsel = new ActionRowBuilder()
                                .addComponents(
                                    new StringSelectMenuBuilder()
                                        .setCustomId("privsel")
                                        .setMinValues(1)
                            ...
                            client.database.updateServerNote(interaction.guild, member.user, note)
                                .then(async (m) => {
                                    client.database.writeDevLog(await client.database.writeLog(interaction.guild, m).catch(err => client.database.writeDevLog(`${err}`)));
                                    await mes.edit({
                                        embeds: [
                                        ...
                                        await mes.edit({
                                            embeds: [
                                                new EmbedBuilder()
                                                    .setColor("Red")
                                                    .setTitle(`${err}`)
                                                    .setDescription("Could not find that Note in the Database. Please contact the Developer if this Issue persists!")
                                                    .setTimestamp()
                                            ],
                                            ephemeral: true
                                        });
                ...
                .then(async (note) => {
                    ...
                    collector.on("collect", async (i) => {
                        ...
                        if (i.customId=="title") {
                            ...
                            note.content = menu.data.fields[2].value;
                            note.private = Boolean(menu.data.fields[1].value);
                            client.database.updateGlobalNote(member.user, note)
                                .then(async (m) => {
                                    client.database.writeDevLog(await client.database.writeLog(interaction.guild, m).catch(err => client.database.writeDevLog(`${err}`)));
                                    await mes.edit({
                                        embeds: [
