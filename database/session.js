...
import { SessionPlayer } from "./session_player.js";
...
class Session {
    ...
    async update(server, user, session) {
        ...
        await query(sql, [session.name, session.description, session.levels, session.players, session.min_runtime, session.max_runtime, session.start_time, session.end_time, session.channel, session.difficulty, session.started, session.ended, server.id, session.id])
