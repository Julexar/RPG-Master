...
class Stats {
    ...
    static async getOne(stat) {
        if (stat.id) {
            const results = await query('SELECT * FROM stats WHERE id = $1', [stat.id]);
        ...
        if (stat.key) {
            const results = await query('SELECT * FROM stats WHERE key = $1', [stat.key]);
        ...
        const results = await query('SELECT * FROM stats WHERE name = $1', [stat.name]);
