...
class SubclassSense {
    static async getAll(sub) {
        const results = await query('SELECT * FROM subclass_senses WHERE sub_id = $1', [sub.id]);
    ...
    static async getOne(sub, sense) {
        if (sense.id) {
            const results = await query('SELECT * FROM subclass_senses WHERE sub_id = $1 AND id = $2', [sub.id, sense.id]);
        ...
        const results = await query('SELECT * FROM subclass_senses WHERE sub_id = $1 AND sense_id = $2', [sub.id, dbSense.id]);
