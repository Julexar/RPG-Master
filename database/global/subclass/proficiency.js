...
class SubclassProficiency {
    static async getAll(sub) {
        const results = await query('SELECT * FROM subclass_proficiencies WHERE sub_id = $1', [sub.id]);
    ...
    static async getOne(sub, prof) {
        if (prof.id) {
            const results = await query('SELECT * FROM subclass_proficiencies WHERE sub_id = $1 AND id = $2', [sub.id, prof.id]);
    ...
    static async update(sub, prof) {
        ...
        await query(sql, [prof.name, prof.expert, prof.type, sub.id, prof.id]);
