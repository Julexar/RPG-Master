...
class Command {
    ...
    async getOne(command: { id?: bigint, name?: string }, type_id: bigint) {
        if (command.id) {
        ...
        const dbType = await CommandType.getOne({ id: type_id });
    ...
    async exists(command: { id?: bigint, name?: string }, type_id: bigint) {
        if (command.id) {
            const results = await query('SELECT * FROM commands WHERE id = $1', [command.id]) as DBCommand[];
    ...
    async add(command: AddCommand, type_id: bigint) {
        if (await this.exists(command, type_id)) throw new DuplicateError('Duplicate Command', 'That Command already exists in the Database!');
    ...
    async remove(command: { id?: bigint, name?: string }, type_id: bigint) {
        if (!(await this.exists(command, type_id))) throw new NotFoundError('Command not found', 'Could not find that Command in the Database!');

        const type = await CommandType.getOne({ id: type_id });
        await query('DELETE FROM commands WHERE id = $1', [command.id]);

        return `Successfully removed ${type.name} \"${command.name}\" from Database`;
    }

    async update(command: DBCommand, type_id: bigint) {
        if (!(await this.exists(command, type_id))) throw new NotFoundError('Command not found', 'Could not find that Command in the Database!');
