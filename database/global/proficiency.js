...
class Proficiency {
    static async getAll() {
        const results = await query('SELECT * FROM proficiencies');
    ...
    static async getOne(prof) {
        if (prof.id) {
            const results = await query('SELECT * FROM proficiencies WHERE id = $1', [prof.id]);
        ...
        if (prof.key) {
            const results = await query('SELECT * FROM proficiencies WHERE key = $1', [prof.key]);
        ...
        const results = await query('SELECT * FROM proficiencies WHERE name = $1', [prof.name]);
