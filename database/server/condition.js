...
class ServerCondition {
    static async getAll(server) {
        ...
        if (results.rows.length === 0) throw new NotFoundError('No Server Conditions found', 'Could not find any conditions for that Server in the Database!');
    ...
    static async restore(server, condition) {
        ...
        if (!(await this.getOne(server, condition)).deleted_at) throw new BadRequestError('Server Condition not deleted', 'That condition for that Server is not deleted!');
